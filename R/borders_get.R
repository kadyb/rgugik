...
borders_get = function(voivodeship = NULL, county = NULL, commune = NULL,
  ...
  if (!is.null(voivodeship)) {
    ...
    TERYT = df_names[sel_vector, "TERC", drop = TRUE]
  ...
  if (!is.null(county)) {
    ...
    TERYT = df_names[sel_vector, "TERYT", drop = TRUE]
  ...
  if (!is.null(TERYT)) {
    ...
    if (nchar(TERYT) == 7) {
      ...
      TERYT = paste0(substr(TERYT, 1, 6), "_", substr(TERYT, 7, 7))
      request = "GetCommuneById"
    }
  ...
  result = "geom_wkb"
  # geom_stack = list()
  geom_stack = st_polygon()

  for (i in seq_len(length(TERYT))) {
    ...
    # geom_stack = c(geom_stack, geom)
    geom_stack = rbind(geom_stack, geom)
