...
<center> ![DTM-DSM](https://upload.wikimedia.org/wikipedia/commons/thumb/6/6c/DTM_DSM.svg/1024px-DTM_DSM.svg.png){ width=50% }
<p style="font-size:8px;">Wikimedia Commons contributors, "File:DTM DSM.svg," Wikimedia Commons, the free media repository, https://commons.wikimedia.org/w/index.php?title=File:DTM_DSM.svg&oldid=475779479 (accessed October 7, 2020).
</p>
</center>
<br>
...
The centroid of the Morasko Meteorite nature reserve has X = 16.895 and Y = 52.487 coordinates in World Geodetic System 1984 (EPSG 4326).
...
We now can combine our models into one stack.
The matrix cells contain exactly the same areas.
...
## x       1 2188 355732     1 ETRS89 / Poland CS92  TRUE     NULL [x]
## y       1 2379 517028    -1 ETRS89 / Poland CS92  TRUE     NULL [y]
...
As a result, we get one complex geometry (*MULTIPOINT*), which we can convert into a simple geometry (*POINT*) consisting of many points.
...
```r
# first layer only
plot(DEM[, , , 1], reset = FALSE, col = terrain.colors(99, alpha = NULL))
plot(line, add = TRUE, col = "red", lwd = 1, key.pos = NULL)
```

![](DEM-unnamed-chunk-16-1.png)
...
plot(nDSM, main = "Trees height [m]", col = hcl.colors(9, palette = "RdYlGn"))
```
