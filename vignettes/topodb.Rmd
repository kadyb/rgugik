...
plot(st_geometry(territory), add = TRUE)
plot(bridges, add = TRUE, pch = 21, cex = 2)
```
...
Let's check the land cover in our county - it is stored in files with the  `PT` prefix.
...
Our `pattern` should look like this: `PT+.+A\\.xml$` - only files containing area (A) land cover (PT) will be listed.
...
The last type of analysis in this vignette is uses spatial buffers.
We want to check how many buildings have access to bus stops within a given distance.
...
plot(st_geometry(bus_stop), add = TRUE, pch = 21, cex = 0.7, col = "blue")
```
