...
```
## Error in ortho_request(krajkowo): could not find function "ortho_request"
```
...
```
## Error in eval(expr, envir, enclos): object 'req_df' not found
```
...
```
## Error in "URL" %in% names(df_req): object 'req_df' not found
```
...
```
## trying to read file: C:\Users\Krzysztof\Desktop\rgugik-master\vignettes\data\69837_329609_N-33-142-B-d-4-2.TIF
```
...
```
## trying to read file: C:\Users\Krzysztof\Desktop\rgugik-master\vignettes\data\69837_329613_N-33-142-B-d-4-4.TIF
```
...
```
## Error in st_mosaic(img1, img2): object 'img1' not found
```
...
```
## Error in st_crs(img) = 2180: object 'img' not found
```
...
```
## Error in st_crop(img, krajkowo): object 'img' not found
```
...
```
## Error in plot(img, rgb = c(1, 2, 3), main = NULL): object 'img' not found
```
...
```
## Error in st_apply(img, MARGIN = c("x", "y"), FUN = calc_ndvi): object 'img' not found
```
...
```
## Error in plot(ndvi, main = "NDVI", col = hcl.colors(10, palette = "RdYlGn")): object 'ndvi' not found
```
