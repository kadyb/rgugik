...
It can be done based on remote sensing data (multichannel orthophotomap) and a simple vegetation index.

**NDVI** (Normalized Difference Vegetation Index) is a simple indicator of vegetation that uses the red and the near infrared channels.
...
Let's load the downloaded orthophotomaps using the `read_stars()` function from the **stars** package, which allows to work with spatiotemporal arrays.
